...
{
    public partial class AdminEditSchool7th : System.Web.UI.Page
    {
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            ...
            if ((Page.IsValid))
            {
                SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["SAConnection"].ConnectionString);
                dynamic id = Request.QueryString["id"];
                dynamic e_id = Session["E_Assessment_id"];
                string SchoolYear = System.Configuration.ConfigurationManager.AppSettings["SchoolYear"];


                int ret = 0, ret2 = 0, ret_e = 0, ret2_e = 0;
                string sql1 = "", sql2 = "", sql1_e="", sql2_e="";
                ...
                try
                {
                    if ((hdnIsComplete.Value == "N" & drpdwnKndrYesNo.SelectedValue == "No"))
                    {
                        ...
                        sql1_e = "UPDATE Schools SET CoCode = @CoCode, DistCode = @DistCode, SchName = @SchName, PhysStreet = @PhysStreet, PhysCity = @PhysCity, PhysZip = @PhysZip, MailStreet = @MailStreet, MailCity = @MailCity, MailZip = @MailZip, SchPhone = @SchPhone, SchType = @SchType, isCharter = @isCharter, Password = @Password, ContactPerson = @ContactPerson, ContactPhone = @ContactPhone, ContactPhoneExt = @ContactPhoneExt, ContactEmail = @ContactEmail, SchAdmin = @SchAdmin, SchEmail = @SchEmail, SuperintendentName = @SuperintendentName, SuperintendentEmail = @SuperintendentEmail, SuperintendentPhone = @SuperintendentPhone, Editdate = Getdate() WHERE id IN (SELECT id FROM Assessments WHERE Assmntid = " + e_id + ")";
                        sql2_e = "UPDATE Assessments SET StudentYesNo = @StudentYesNo, Reason = @Reason, isComplete = 'Y', SubmitDate = @SubmitDate, ReportedPerson = @ReportedPerson, ReportedPhone = @ReportedPhone, ReportedPhoneExt = @ReportedPhoneExt, ReportedEmail = @ReportedEmail,  AllImm = @AllImm, MedExmp = @MedExmp, BeleExmp = @BeleExmp, NoImm = @NoImm, TotNo = @TotNo, Polio = @Polio, DTP_DTAP_DT = @DTP, MMRDose1 = @MMRDose1, MMRDose2 = @MMRDose2, HepB = @HepB, VZV = @VZV, Memo = @Memo,  LhdReviseDate = @LhdReviseDate, Editdate = Getdate() WHERE Assmntid = " + e_id + " AND SchoolYear = " + SchoolYear;
                        ...
                        sql1_e = "UPDATE Schools SET CoCode = @CoCode, DistCode = @DistCode, SchName = @SchName, PhysStreet = @PhysStreet, PhysCity = @PhysCity, PhysZip = @PhysZip, MailStreet = @MailStreet, MailCity = @MailCity, MailZip = @MailZip, SchPhone = @SchPhone, SchType = @SchType, isCharter = @isCharter, ContactPerson = @ContactPerson, ContactPhone = @ContactPhone, ContactPhoneExt = @ContactPhoneExt, ContactEmail = @ContactEmail, SchAdmin = @SchAdmin, SchEmail = @SchEmail, SuperintendentName = @SuperintendentName, SuperintendentEmail = @SuperintendentEmail, SuperintendentPhone = @SuperintendentPhone, Editdate = Getdate()  WHERE id IN (SELECT id FROM Assessments WHERE Assmntid = " + e_id + ")";
                        sql2_e = "UPDATE Assessments SET StudentYesNo = @StudentYesNo, Reason = @Reason, ReportedPerson = @ReportedPerson, ReportedPhone = @ReportedPhone, ReportedPhoneExt = @ReportedPhoneExt, ReportedEmail = @ReportedEmail, Memo = @Memo,  LhdReviseDate = @LhdReviseDate, Editdate = Getdate() WHERE Assmntid = " + e_id + " AND SchoolYear = " + SchoolYear;
                    ...
                    SqlCommand cmd_e = new SqlCommand(sql1_e, con);
                    SqlCommand cmd2_e = new SqlCommand(sql2_e, con);
                    ...
                    cmd_e.Parameters.AddWithValue("@CoName", txtCounty.Text);
                    cmd_e.Parameters.AddWithValue("@DistName", txtDistrict.Text);
                    cmd_e.Parameters.AddWithValue("@SchName", txtSchoolName.Text);
                    cmd_e.Parameters.AddWithValue("@PhysStreet", txtPhyAddress.Text);
                    cmd_e.Parameters.AddWithValue("@PhysCity", txtPhyCity.Text);
                    cmd_e.Parameters.AddWithValue("@PhysZip", txtPhyZip.Text);
                    cmd_e.Parameters.AddWithValue("@MailStreet", txtMailAddress.Text);
                    cmd_e.Parameters.AddWithValue("@MailCity", txtMailCity.Text);
                    cmd_e.Parameters.AddWithValue("@MailZip", txtMailZip.Text);
                    cmd_e.Parameters.AddWithValue("@SchPhone", txtSchoolPhone.Text);
                    cmd_e.Parameters.AddWithValue("@SchAdmin", txtSchAdmin.Text);
                    cmd_e.Parameters.AddWithValue("@SchEmail", txtSchEmail.Text);
                    ...
                    cmd_e.Parameters.AddWithValue("@ContactPerson", txtContactPerson.Text);
                    cmd_e.Parameters.AddWithValue("@ContactPhone", txtContactPhone.Text + txtContactPhone_1.Text + txtContactPhone_2.Text);
                    cmd_e.Parameters.AddWithValue("@ContactPhoneExt", txtContactPhoneExt.Text);
                    cmd_e.Parameters.AddWithValue("@ContactEmail", txtContactEmail.Text);
                    cmd2_e.Parameters.AddWithValue("@ReportedPerson", txtFormPerson.Text);
                    cmd2_e.Parameters.AddWithValue("@ReportedPhone", txtFormPhone.Text + txtFormPhone_1.Text + txtFormPhone_2.Text);
                    cmd2_e.Parameters.AddWithValue("@ReportedPhoneExt", txtFormPhoneExt.Text);
                    cmd2_e.Parameters.AddWithValue("@ReportedEmail", txtFormEmail.Text);
                    cmd2_e.Parameters.AddWithValue("@StudentYesNo", drpdwnKndrYesNo.SelectedValue);
                    cmd2_e.Parameters.AddWithValue("@Reason", drpReason.SelectedValue);
                    cmd2_e.Parameters.AddWithValue("@Memo", txtMemo.Text);
                    cmd_e.Parameters.AddWithValue("@SuperintendentName", txtSuperintendentName.Text);
                    cmd_e.Parameters.AddWithValue("@SuperintendentEmail", txtSuperintendentEmail.Text);
                    cmd_e.Parameters.AddWithValue("@SuperintendentPhone", txtSuperintendentPhone.Text);
                    cmd2_e.Parameters.AddWithValue("@LhdReviseDate", curDateTime);
                    cmd2_e.Parameters.AddWithValue("@SubmitDate", curDateTime);

                    cmd_e.Parameters.AddWithValue("@Password", "0000");

                    cmd2_e.Parameters.AddWithValue("@AllImm", 0);
                    cmd2_e.Parameters.AddWithValue("@MedExmp", 0);
                    cmd2_e.Parameters.AddWithValue("@BeleExmp", 0);
                    cmd2_e.Parameters.AddWithValue("@NoImm", 0);
                    cmd2_e.Parameters.AddWithValue("@TotNo", 0);
                    cmd2_e.Parameters.AddWithValue("@Polio", 0);
                    cmd2_e.Parameters.AddWithValue("@DTP", 0);
                    cmd2_e.Parameters.AddWithValue("@MMRDose1", 0);
                    cmd2_e.Parameters.AddWithValue("@MMRDose2", 0);
                    cmd2_e.Parameters.AddWithValue("@HepB", 0);
                    cmd2_e.Parameters.AddWithValue("@VZV", 0);
                    ...
                    */
                    ...
                    ret2_e = cmd2_e.ExecuteNonQuery();

                    if ((ret == 1) && (ret2 == 1) && (ret_e == 1) && (ret2_e == 1))
                    {
                        /*
