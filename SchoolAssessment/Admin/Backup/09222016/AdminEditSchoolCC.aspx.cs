...
{
    public partial class AdminEditSchoolCC : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if ((Page.IsPostBack == false & !string.IsNullOrEmpty(Request.QueryString["id"])))
            {
                ...
                string sql = "SELECT A.*, C.*, S.*, D.* FROM Assessments A INNER JOIN Schools S ON S.SchCode = A.SchCode LEFT JOIN COUNTIES C ON C.CoCode = S.CoCode LEFT JOIN Districts D ON D.DistCode = S.DistCode WHERE A.Assmntid = " + id + "";
                ...
                try
                {
                    ...
                    if ((Session["AdminUserType"] == "LHD" && Session["AdminCoCode"] == "59" && txtCountyCode.Text != "01" && txtPhyCity.Text != "BERKELEY"))
                    {
                        Response.Redirect("AdminLogin.aspx?reason=InsufficientPrivileges", true);
                    }
                    else if ((Session["AdminUserType"] == "LHD" && Session["AdminCoCode"] == "59" && txtCountyCode.Text == "01" && txtPhyCity.Text == "BERKELEY"))
                    {
                        // do nothing, this is okay
                    }
                    else if ((Session["AdminUserType"] == "LHD" && Session["AdminCoCode"] == "01" && txtCountyCode.Text == "01" && txtPhyCity.Text == "BERKELEY"))
                    {
                        Response.Redirect("AdminLogin.aspx?reason=InsufficientPrivileges", true);
                    }
                    else if ((Session["AdminUserType"] == "LHD" && Session["AdminCoCode"] == "60" && txtCountyCode.Text != "19" && txtPhyCity.Text != "LONG BEACH"))
                    {
                        Response.Redirect("AdminLogin.aspx?reason=InsufficientPrivileges", true);
                    }
                    else if ((Session["AdminUserType"] == "LHD" && Session["AdminCoCode"] == "60" && txtCountyCode.Text == "19" && txtPhyCity.Text == "LONG BEACH"))
                    ...
                    else if ((Session["AdminUserType"] == "LHD" && Session["AdminCoCode"] == "19" && txtCountyCode.Text == "19" && txtPhyCity.Text == "LONG BEACH"))
                    {
                        Response.Redirect("AdminLogin.aspx?reason=InsufficientPrivileges", true);
                    }
                    else if ((Session["AdminUserType"] == "LHD" && txtCountyCode.Text != Session["AdminCoCode"]))
                    {
                        Response.Redirect("AdminLogin.aspx?reason=InsufficientPrivileges", true);
                    }
                    else if ((Session["AdminUserType"] == "FIELDREP" && region != Session["AdminRegionCode"]))
                    {
                        Response.Redirect("AdminLogin.aspx?reason=InsufficientPrivileges", true);
                    }

                    if ((Session["AdminUserType"] != "ADMIN"))
                    {
            ...
            if ((Page.IsValid))
            {
                ...
                try
                {
                    if ((hdnIsComplete.Value == "N" & drpdwnKndrYesNo.SelectedValue == "No"))
                    {
                        ...
                        sql1 = "UPDATE Schools SET CoCode = @CoCode, CoName = @CoName, SchName = @SchName, PhysStreet = @PhysStreet, PhysCity = @PhysCity, PhysZip = @PhysZip, MailStreet = @MailStreet, MailCity = @MailCity, MailZip = @MailZip, SchPhone = @SchPhone, SchType = @SchType, isCharter = @isCharter, Password = @Password, ContactPerson = @ContactPerson, ContactPhone = @ContactPhone, ContactPhoneExt = @ContactPhoneExt, ContactEmail = @ContactEmail, SchAdmin = @SchAdmin, SchEmail = @SchEmail, SuperintendentName = @SuperintendentName, SuperintendentEmail = @SuperintendentEmail, SuperintendentPhone = @SuperintendentPhone WHERE id IN (SELECT id FROM Assessments WHERE Assmntid = " + id + ")";
                        sql2 = "UPDATE Assessments SET StudentYesNo = @StudentYesNo, Reason = @Reason, isComplete = 'Y', SubmitDate = @SubmitDate, ReportedPerson = @ReportedPerson, ReportedPhone = @ReportedPhone, ReportedPhoneExt = @ReportedPhoneExt, ReportedEmail = @ReportedEmail,  AllImm = @AllImm, MedExmp = @MedExmp, BeleExmp = @BeleExmp, NoImm = @NoImm, TotNo = @TotNo, Polio = @Polio, DTP_DTAP_DT = @DTP, MMRDose1 = @MMRDose1, MMRDose2 = @MMRDose2, HepB = @HepB, VZV = @VZV, Memo = @Memo,  LhdReviseDate = @LhdReviseDate WHERE Assmntid = " + id + " AND SchoolYear = " + SchoolYear;
                        ...
                        sql1 = "UPDATE Schools SET CoCode = @CoCode, SchName = @SchName, PhysStreet = @PhysStreet, PhysCity = @PhysCity, PhysZip = @PhysZip, MailStreet = @MailStreet, MailCity = @MailCity, MailZip = @MailZip, SchPhone = @SchPhone, SchType = @SchType, isCharter = @isCharter, ContactPerson = @ContactPerson, ContactPhone = @ContactPhone, ContactPhoneExt = @ContactPhoneExt, ContactEmail = @ContactEmail, SchAdmin = @SchAdmin, SchEmail = @SchEmail, SuperintendentName = @SuperintendentName, SuperintendentEmail = @SuperintendentEmail, SuperintendentPhone = @SuperintendentPhone  WHERE id IN (SELECT id FROM Assessments WHERE Assmntid = " + id + ")";
                        sql2 = "UPDATE Assessments SET StudentYesNo = @StudentYesNo, Reason = @Reason, ReportedPerson = @ReportedPerson, ReportedPhone = @ReportedPhone, ReportedPhoneExt = @ReportedPhoneExt, ReportedEmail = @ReportedEmail, Memo = @Memo,  LhdReviseDate = @LhdReviseDate WHERE Assmntid = " + id + " AND SchoolYear = " + SchoolYear;
