...
namespace SchoolAssessment.Models.KG
{
    public partial class WebForm1 : System.Web.UI.Page
    {
        ...
        {            
            SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["SAConnection"].ConnectionString);
            SqlCommand cmd = new SqlCommand("SELECT DISTINCT CoName FROM Counties WHERE CoName NOT IN ( 'NOacNE', 'BERKELEY CITY', 'LONG BEACH CITY', 'PASADENA') ORDER BY CoName ASC", con);
        ...
        private void FillSchool()
        {
            ...
            try
            {
                ...
                {
                    //sql = "SELECT distinct [SchName] FROM K_Assessment where SchoolYear = '" + schoolyear + "' and CoName = '" + county + "' and SchType = '" + type + "' and DistName = '" + district + "' ORDER BY [SchName] asc";
                    sql = "SELECT DISTINCT [SchName] FROM Schools S INNER JOIN[Districts] D ON D.DistCode = S.DistCode INNER JOIN[Counties] C ON C.CoCode = S.Cocode WHERE Cohort = 'K' and C.CoName = '" + county + "' and SchType = '" + type + "' and D.DistName = '" + district + "' and S.EditDate > '" + ActiveSchool + "' ORDER BY[SchName] ASC";

                }
                else {
                    //sql = "SELECT distinct [SchName] FROM K_Assessment where SchoolYear = '" + schoolyear + "' and CoName = '" + county + "' and SchType = '" + type + "' ORDER BY [SchName] asc";
                    sql = "SELECT DISTINCT [SchName] FROM Schools S INNER JOIN[Counties] C ON C.CoCode = S.Cocode WHERE Cohort = 'K' and CoName = '" + county + "' and SchType = '" + type + "' and S.EditDate > '" + ActiveSchool + "' ORDER BY[SchName] ASC";
                }
        ...
        private void FillSchoolCode()
        {
            ...
            try
            {
                ...
                if ((type == "PUBLIC"))
                {
                    sql = "SELECT DISTINCT SchCode FROM SCHOOLS S INNER JOIN [Districts] D ON D.DistCode = S.DistCode INNER JOIN[Counties] C ON C.CoCode = S.Cocode WHERE Cohort = 'K' and C.CoName = '" + county + "' and[SchType] = '" + type + "'and D.DistName = '" + district + "' and SchName = '" + schoolname + "' and S.EditDate > '" + ActiveSchool + "' ORDER BY SchCode";

                }
                else {
                    sql = "SELECT distinct SchCode FROM SCHOOLS S INNER JOIN[Counties] C ON C.CoCode = S.Cocode WHERE Cohort = 'K' and C.CoName ='" + county + "' and [SchType] = '" + type + "'and SchName = '" + schoolname + "' and S.EditDate > '" + ActiveSchool + "' ORDER BY SchCode";
                }
        ...
        private void CheckPassword(ref bool pwdstatus)
        {
            ...
            try
            {
                ...
                if ((sdr.HasRows == true))
                {
                    ...
                    pwdstatus = false;
                    // assign session variable here for school does not exist
        ...
        private void SetFirstGradeAsmntID()
        {
            string FirstGradeID = "";
            SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["SAConnection"].ConnectionString);
            String schoolCode = txtSchoolCode.Text;
            string SchoolYear = System.Configuration.ConfigurationManager.AppSettings["SchoolYear"];
            string sql = "SELECT TOP 1 a.Assmntid FROM Schools S INNER JOIN Assessments A ON S.id = a.id where S.Cohort = 'F' and S.SchCode = '" + schoolCode + "' AND A.schoolYear = " + SchoolYear;
