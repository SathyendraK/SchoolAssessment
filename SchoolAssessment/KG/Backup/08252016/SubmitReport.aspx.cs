...
namespace SchoolAssessment.KG
{
    public partial class SubmitReport : System.Web.UI.Page
    {
        ...
        {
            if ((Convert.ToInt32(txtPermMedExmp.Text) + Convert.ToInt32(txtBelExmp.Text) + Convert.ToInt32(TextIEPServices.Text) + Convert.ToInt32(TextIndependentStudy.Text) + Convert.ToInt32(TextHomeBasedPrivate.Text) + Convert.ToInt32(txtNoimm.Text) + Convert.ToInt32(TextMedExmption.Text) + Convert.ToInt32(TextEnrolledButNotAttending.Text)) > (Convert.ToInt32(txtPolio.Text) + Convert.ToInt32(txtDtp.Text) + Convert.ToInt32(txtMMR2.Text) + Convert.ToInt32(txtHepb.Text) + Convert.ToInt32(txtVZV.Text)))
            {
                args.IsValid = false;
            }
            else {
            ...
            if (((Session["K_Assessment_id"] == null)))
            {
                ...
                { 
                   ...
                   if (Object.ReferenceEquals(null, SesAdminUserType))
                        ...
                        sql = "UPDATE Assessments SET isComplete = 'Y', SubmitDate = @SubmitDate, ReviseDate = @ReviseDate, AllImm = @AllImm, MedExmp = @MedExmp, BeleExmp = @BeleExmp, NoImm = @NoImm, TotNo = @TotNo, Polio = @Polio, DTP_DTAP_DT = @DTP, MMRDose2 = @MMRDose2, HepB = @HepB, VZV = @VZV, EnrolledButNotAttending = @EnrolledButNotAttending, IEPService = @IEP, IndependentStudy = @IndependentStudy,  HomeBasedPrivate =  @HomeBased , TempMedExemption = @TMPMedExmp WHERE id = '" + id + "' AND SchoolYear = '" + SchoolYear + "' ";
                        AdminReviseDate = reviseDate.ToString();
                        LHDReviseDate = hdnLhdReviseDate.Value;
                    }
                    else {
                        sql = "UPDATE Assessments SET isComplete = 'Y', SubmitDate = @SubmitDate, LhdReviseDate = @LhdReviseDate, AllImm = @AllImm, MedExmp = @MedExmp, BeleExmp = @BeleExmp, NoImm = @NoImm, TotNo = @TotNo, Polio = @Polio, DTP_DTAP_DT = @DTP, MMRDose2 = @MMRDose2, HepB = @HepB, VZV = @VZV, EnrolledButNotAttending = @EnrolledButNotAttending, IEPService = @IEP, IndependentStudy = @IndependentStudy,  HomeBasedPrivate =  @HomeBased , TempMedExemption = @TMPMedExmp WHERE id = '" + id + "' AND SchoolYear = '" + SchoolYear + "' ";
