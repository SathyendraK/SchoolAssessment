...
{
    public partial class SubmitFirstGradeReport : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if ((Page.IsPostBack == false))
            {
                ...
                string sql = "SELECT S.*, C.CoName, A.* FROM[dbo].[Assessments] A INNER JOIN Schools S on A.id = S.id INNER JOIN Counties C on C.CoCode = S.CoCode WHERE A.Assmntid = '" + id_Fgrade + "'";
                    ...
                    while (reader.Read())
                    {
                        ...
                        */
                        ...
                        if ((String)reader["StudentYesNo"].ToString() == "No")
                        {
                            ...
                            //txtBelExmp.Text = "0"; No longer collecting from 2017-2018 
                            txtNoimm.Text = "0";
                            txtPolio.Text = "0";
                            txtDtp.Text = "0";
                            txtMMR2.Text = "0";
        ...
        private void Save()
        {
            ...
            else
            {
                ...
                // Getting First Grade Info
                dynamic id = Session["F_Assessment_id"];
                ...
                try
                {
                    ...
                    if (Object.ReferenceEquals(null, SesAdminUserType))
                    {
                        ...
                        sql = "UPDATE Assessments SET isComplete = 'Y', SubmitDate = @SubmitDate, ReviseDate = @ReviseDate, AllImm = @AllImm, MedExmp = @MedExmp, BeleExmp = NULL, NoImm = @NoImm, TotNo = @TotNo, Polio = @Polio, DTP_DTAP_DT = @DTP, MMRDose2 = @MMRDose2, HepB = @HepB, VZV = @VZV, EnrolledButNotAttending = @EnrolledButNotAttending, IEPService = @IEP, IndependentStudy = @IndependentStudy,  HomeBasedPrivate =  @HomeBased , TempMedExemption = @TMPMedExmp WHERE Assmntid = '" + id + "'";
                        AdminReviseDate = reviseDate.ToString();
                        LHDReviseDate = hdnLhdReviseDate.Value;
                    }
                    else
                    {
                        sql = "UPDATE Assessments SET isComplete = 'Y', SubmitDate = @SubmitDate, LhdReviseDate = @LhdReviseDate, AllImm = @AllImm, MedExmp = @MedExmp, BeleExmp = NULL, NoImm = @NoImm, TotNo = @TotNo, Polio = @Polio, DTP_DTAP_DT = @DTP, MMRDose2 = @MMRDose2, HepB = @HepB, VZV = @VZV, EnrolledButNotAttending = @EnrolledButNotAttending, IEPService = @IEP, IndependentStudy = @IndependentStudy,  HomeBasedPrivate =  @HomeBased , TempMedExemption = @TMPMedExmp WHERE Assmntid = '" + id + "'";
    ...
    */
                        ...
                        // No --> Update First Grade Data with 0 and Step 4
        ...
        private void logaudittrail(string fieldname, string fromval, string toval)
        {
            ...
            try
            {
                ...
                cmd.Parameters.AddWithValue("@ScreenName", "FSubmitReport");
        ...
        public void KindergartenSum(object source, ServerValidateEventArgs args)
        {
            ...
            if (((Convert.ToInt32(txtAllimm.Text) + Convert.ToInt32(txtPermMedExmp.Text) + Convert.ToInt32(TextIEPServices.Text) + Convert.ToInt32(TextIndependentStudy.Text) + Convert.ToInt32(TextHomeBasedPrivate.Text) + Convert.ToInt32(txtNoimm.Text) + Convert.ToInt32(TextMedExmption.Text) + Convert.ToInt32(TextEnrolledButNotAttending.Text)) != Convert.ToInt32(txttotno.Text)))
            {
                args.IsValid = false;
            }
            else
        ...
        {
            if ((Convert.ToInt32(txtPermMedExmp.Text) + Convert.ToInt32(TextIEPServices.Text) + Convert.ToInt32(TextIndependentStudy.Text) + Convert.ToInt32(TextHomeBasedPrivate.Text) + Convert.ToInt32(txtNoimm.Text) + Convert.ToInt32(TextMedExmption.Text) + Convert.ToInt32(TextEnrolledButNotAttending.Text)) > (Convert.ToInt32(txtPolio.Text) + Convert.ToInt32(txtDtp.Text) + Convert.ToInt32(txtMMR2.Text) + Convert.ToInt32(txtHepb.Text) + Convert.ToInt32(txtVZV.Text)))
            {
                args.IsValid = false;
            }
            else
        ...
        protected void ImgBtnSubmit_Click(object sender, System.Web.UI.ImageClickEventArgs e)
        {
            ...
            else
            {
                ...
                if ((Page.IsValid))
                {
                    // Popup window to confirm the submission
                    /*
                    var confirmResult = MessageBox.Show("Are you ready to submit the Report?",
                                            "Confirm Submission",
