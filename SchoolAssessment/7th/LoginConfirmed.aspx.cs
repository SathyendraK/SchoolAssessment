        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (((Session["7th_Assessment_id"] == null)))
            {
                ...
                */
                ...
                string sql_e = "SELECT S.*, C.CoName, ISNULL(D.DistName, '') AS 'DistName', A.StudentYesNo, A.Reason, A.HomeSchl, A.VirtualSchl, A.isComplete FROM Schools S INNER JOIN Counties C on S.CoCode = C.CoCode LEFT OUTER JOIN Districts D ON S.DistCode = D.DistCode INNER JOIN Assessments A ON A.ID = S.id WHERE A.Assmntid = '" + id_e + "'";
                ...
                try
                {
                    ...
                    while (reader.Read())
                    {
                        ...
                        // Commented out on 08/31/2017, 06/19/2018, 06/27/2019, 08/11/2021
                        // Uncommented on 02/14/2018, 11/30/2018, 12/05/2019, 03/15/2021, 03/10/22
                        ...
                        */
                        ...
                        if ((drpdwnKndrYesNo.SelectedValue == "No"))
                        {
                            ...
                            //drphome.Enabled = false;  Commented out on 11/04/2020
                            //drpvirtual.Enabled = false; Commented out on 11/04/2020

                            // 7th grade N --> 8th grade Grey out
                            drpdwn8thYesNo.SelectedValue = "No";
                            drpReason8th.Enabled = true;
                            valReason8th.Enabled = true;
                        }
                        else {
                            ...
                            //drphome.Enabled = true;  Commented out on 11/04/2020
                            //drpvirtual.Enabled = true; Commented out on 11/04/2020
                        }
                    ...
                    while (reader_e.Read())
                    {
                        hdn8thYesNo.Value = (String)reader_e["StudentYesNo"].ToString();
                        hdn8thReason.Value = reader_e["Reason"].ToString();

                        if ((Page.IsPostBack == false))
                        {
                            drpdwn8thYesNo.SelectedValue = (String)reader_e["StudentYesNo"].ToString();
                            drpReason8th.SelectedValue = reader_e["Reason"].ToString();
                        }

                        if ((drpdwn8thYesNo.SelectedValue == "No"))
                        {
                            //lblConfirm.Visible = true;
                            drpReason8th.Enabled = true;
                            valReason8th.Enabled = true;
                        ...
                        {
                            lblConfirm.Visible = false;
                            drpReason8th.SelectedValue = "";
                            drpReason8th.Enabled = false;
                            valReason8th.Enabled = false;
        ...
        private void Save()
        {
            ...
            try
            {
                ...
                if ((drpdwnKndrYesNo.SelectedValue == "Yes"))
                {
                    ...
                    if (drpdwn8thYesNo.SelectedValue == "Yes")
                    {
                        sql3 = "UPDATE Assessments SET StudentYesNo = 'Yes', Reason = NULL, HomeSchl = 'Not Collected', VirtualSchl = 'Not Collected', EditDate = GetDate()  WHERE Assmntid = '" + id_e + "'";
                        cmd3 = new SqlCommand(sql3, con);
                    }
                    else
                    {
                        sql3 = "UPDATE Assessments SET StudentYesNo = 'No', Reason = @Reason, HomeSchl = 'No', VirtualSchl = 'No', EditDate = GetDate() WHERE Assmntid = '" + id_e + "'";
                        cmd3 = new SqlCommand(sql3, con);
                        cmd3.Parameters.AddWithValue("@Reason", drpReason8th.SelectedValue);
                    }
