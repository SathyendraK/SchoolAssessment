    ...
    public partial class ReportContactInfo : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if ((Page.IsPostBack == false))
            {
                ...
                string sql_e = "SELECT S.*, C.CoName, A.StudentYesNo, A.Reason, A.ReportedPerson, A.ReportedPhone, A.ReportedPhoneExt, A.ReportedEmail, A.IsComplete FROM[dbo].[Assessments] A INNER JOIN Schools S on A.id = S.id INNER JOIN Counties C on C.CoCode = S.CoCode WHERE A.Assmntid = '" + id_e + "' ";
            ...
            if (((Session["7th_Assessment_id"] == null)))
            {
                ...
                try
                {
                    ...
                    */
                    ...
                    /* 8th grader Save*/
                    sql3 = "UPDATE Assessments SET EditDate = GetDate(), ReportedPerson = @FormPerson, ReportedPhone = @FormPhone, ReportedPhoneExt = @FormPhoneExt, ReportedEmail = UPPER(@FormEmail) WHERE Assmntid = '" + id_e + "'";
                    SqlCommand cmd3 = new SqlCommand(sql3, con);
                    cmd3.Parameters.AddWithValue("@FormPerson", txtStaffPrsn.Text);
                    cmd3.Parameters.AddWithValue("@FormPhone", txtStaffPhNo.Text + txtStaffPhNo_1.Text + txtStaffPhNo_2.Text);
                    cmd3.Parameters.AddWithValue("@FormPhoneExt", txtStaffPhNoExt.Text);
                    cmd3.Parameters.AddWithValue("@FormEmail", txtmail.Text);

                    sql4 = "UPDATE Schools SET EditDate = GetDate(), ContactPerson = @ContactPerson, ContactPhone = @ContactPhone, ContactPhoneExt = @ContactPhoneExt, ContactEmail = UPPER(@ContactEmail) WHERE id IN (SELECT id FROM Assessments WHERE Assmntid = '" + id_e + "')";
                    SqlCommand cmd4 = new SqlCommand(sql4, con);
                    cmd4.Parameters.AddWithValue("@ContactPerson", txtfcperson.Text);
                    cmd4.Parameters.AddWithValue("@ContactPhone", txtfcNumber.Text + txtfcNumber_1.Text + txtfcNumber_2.Text);
                    cmd4.Parameters.AddWithValue("@ContactPhoneExt", txtfcNumberExt.Text);
                    cmd4.Parameters.AddWithValue("@ContactEmail", txtfcemail.Text);

                    /*
                    cmd.Parameters("@id").Value = Session["7th_Assessment_id"];
        ...
        private void logaudittrail(string fieldname, string fromval, string toval)
        {
            ...
            try
            {
                ...
                cmd.Parameters.AddWithValue("@ScreenName", "7thReportContactInfo");
        ...
        protected void CustomValidator_txtEmailValidateSpace(object source, System.Web.UI.WebControls.ServerValidateEventArgs args)
        {
            ...
            else if (!IsEmailValid(txtmail.Text)) {
        ...
        protected void CustomValidator2_ServerValidateSpace(object source, System.Web.UI.WebControls.ServerValidateEventArgs args)
        {
            if (txtconfirmmail.Text.Length == 0)
            {

                args.IsValid = false;
                CustomValidator2.ErrorMessage = "School Staff Member Completing This Form Confirm Email - Required.";
            ...
            else if (Regex.IsMatch(txtconfirmmail.Text, @"\s+"))
            {
                ...
                args.IsValid = false;
                CustomValidator2.ErrorMessage = "Please remove space from Staff Member Completing This Form Confirm Email.";
            ...
            else if (!IsEmailValid(txtconfirmmail.Text.ToString()))
            {
                ...
                args.IsValid = false;
                CustomValidator2.ErrorMessage = "School Staff Member Completing This Form Confirm Email - Invalid format.";
            }
            else if (txtconfirmmail.Text != txtmail.Text)
            {

                args.IsValid = false;
                CustomValidator2.ErrorMessage = "School Staff Member Completing This Form Confirm Email - Does not match email1";
            }
            else
            {
                args.IsValid = true;
        ...
        protected void CustomValidator3_ServerValidate(object source, System.Web.UI.WebControls.ServerValidateEventArgs args)
        {
            ...
            {
                args.IsValid = false;
                CustomValidator3.ErrorMessage = "Designated School Contact Email - Required.";
            }
            else if (Regex.IsMatch(txtfcemail.Text, @"\s+"))
            {
                args.IsValid = false;
                CustomValidator3.ErrorMessage = "Please remove space from Designated School Contact Email.";
            }
            else if (!IsEmailValid(txtfcemail.Text.ToString()))
            {
                args.IsValid = false;
                CustomValidator3.ErrorMessage = "Designated School Contact Email - Invalid format.";
            }
            else
            {
                args.IsValid = true;
